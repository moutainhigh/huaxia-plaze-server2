...
//@Component
@Service
@WebService(serviceName = "WST000203", endpointInterface = "com.huaxia.plaze.modules.MessageWebService")
public class HongKongAndMacaoWebService implements MessageWebService {
/**
 ...
 */
	...
	private final static Logger logger = LoggerFactory.getLogger(HongKongAndMacaoWebService.class);
	...
	public String invoke(String jsonRequest) {
		...
		if ("1".equals(nciicForeignerRequest.getQueryMode())) {
			Client client = null;
			try {
				client = new Client(new URL(AppConfig.getInstance().getValue("police.hongkongandmacao.dmz.webservice.url")));
				...
				if (flag) {
					...
					if (nciicForeignerInfo == null) {
						logger.error("港澳人永久居留证】解析dmz报文异常 trn_id = " + nciicForeignerRequest.getTrnId());
					}else{
						...
						if (rowChange < 1) {
							logger.error("【港澳人永久居留证】持久化返回结果失败");
						}
					}
				} else {
				...
				logger.error("【港澳人永久居留证 】请求dmz查询异常{}", e);
				return ReturnJsonUtil.getBackJson(nciicForeignerRequest.getTrnId(), ErrorCode.DEAL_EX, "");
			} finally {
				if (client != null) {
					client.close();
