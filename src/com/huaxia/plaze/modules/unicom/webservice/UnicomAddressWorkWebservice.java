...
@Service
@WebService(serviceName = "WST001102", endpointInterface = "com.huaxia.plaze.modules.MessageWebService")
public class UnicomAddressWorkWebservice implements MessageWebService{

	private final static Logger logger = LoggerFactory.getLogger(UnicomAddressWorkWebservice.class);
...
//		logger.info("接收到联通地址类(工作地址)请求json>>"+jsonRequest);
...
//		logger.info("联通地址类(工作地址)请求DMZjson>>"+jsonObject);
		...
		uniaddinforRequest.setApiKey("360015"); // 产品ID
		...
		try{
			client = new Client(new URL(AppConfig.getInstance().getValue("unicom.address.work.dmz.webservice.url")));
			// 请求报文添加了运营商类别
			Object[] obj = new Object[] { jsonObject.toString() };
			Object[] result = client.invoke("invoke", obj);
			
//			logger.info("工作地址类返回结果>>"+result);
...
//				logger.info("联通地址类(工作地址)DMZ返回结果报文>>"+jsonResponse);
				...
				unicomAddress.setApiKey("360015"); //产品ID
				...
				if(jsMessage != null && !"".equals(jsMessage)){
					...
					if(jsMessage.containsKey("checkResult") && jsMessage.getString("checkResult")!=null
						...
						// 获取校验结果中文描述
						checkDesc = UniaddinforUtil.gainCheckDesc(checkResult, carrier, "360015");
			...
			e.printStackTrace();
			logger.error("联通地址类信息(工作地址)请求查询异常,交易编号:"+trnId, e);
			return response;
		}
